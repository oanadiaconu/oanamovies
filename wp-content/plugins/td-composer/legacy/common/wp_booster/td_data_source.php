...
//            $tax_in = array();
    ...
    static function &get_wp_query ( $atts = '', $paged = '', $type = '' ) { // by ref
    	...
    	if ( $type === 'products' ) {
		    ...
		    global $td_woo_loop_products_atts_ds;
		    $td_woo_loop_products_atts_ds = array();
		    ...
		    if ( ! empty( $products_ids ) ) {
			    ...
			    if ( ! empty( $products_in ) ) {
				    $td_woo_loop_products_atts_ds['post__in'] = $products_in;
			    }

			    if ( ! empty( $products_not_in ) ) {
				    $td_woo_loop_products_atts_ds['post__not_in'] = $products_not_in;
			    }
		    ...
		    $td_woo_loop_products_atts_ds['offset'] = $offset;

		    if ( $page > 1 ) {
			    $td_woo_loop_products_atts_ds['paged'] = $page;
		    }
		    ...
		    if ( defined('TD_WOO') && class_exists( 'td_woo_util' ) ) {
		    	...
		    	if ( td_woo_util::is_ajax() || ( tdb_state_template::get_template_type() === 'woo_shop_base' || !tdb_state_template::get_template_type() ) ) {
				    if ( $block_type === 'td_woo_products_loop' && !empty( $atts['td_woo_attributes_filters'] ) && is_array( $atts['td_woo_attributes_filters'] ) ) {
					    ...
					    global $td_woo_attributes_filters_ds;
					    $td_woo_attributes_filters_ds = array();
		    ...
		    add_filter( 'woocommerce_shortcode_products_query', function ( $query_args, $attributes, $type ) {
			    global $td_woo_loop_products_atts_ds, $td_woo_attributes_filters_ds;
			    ...
			    return array_merge( $query_args, $td_woo_loop_products_atts_ds );
		    }, 10, 3 );
