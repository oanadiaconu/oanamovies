                            ...
                            <div class="tda-content-wrap">
                                <div class="tda-content">
                                    <div class="tda-btns-wrap">
                                        <a href="#" class="tda-btn tda-btn-auto" title="This will perform an automatic test to find missing CSS rules applied by user or javascript interaction">Analyze CSS</a>
                                        ...
                                        <a href="#" class="tda-btn tda-btn-save tda-save-essential-css" title="This will generate the final CSS file for this page and also will split the rules in Critical CSS to be loaded inline in head">
                                            <span class="tda-submit">Save Found CSS</span>
                                            <span class="tda-saving"><span class="tda-dots"></span>Saving</span>
                                            <span class="tda-done"><svg version="1.1" xmlns="http://www.w3.org/2000/svg" width="1229" height="1024" viewBox="0 0 1229 1024"><path d="M409.6 730.453l-286.72-286.72-95.573 95.573 382.293 382.293 819.2-819.2-95.573-95.573-723.627 723.627z"></path></svg>CSS Saved</span>
                                        </a>
...
function td_analyze_css_admin_bar_button() {
    ...
    if (!is_super_admin() || !is_admin_bar_showing() || is_admin() || 'page' !== $post->post_type || 'publish' !== $post->post_status) {
        return;
    }
...
add_filter('get_post_metadata', 'tda_filter_get_post_metadata', 10, 4);
function tda_filter_get_post_metadata( $null, $object_id, $meta_key, $single) {
    if ( td_util::tdc_is_live_editor_iframe() && ('tda_essential_css' === $meta_key )) {
        return false;
    }
    return null;
}
...
if ( in_array( version_compare(get_bloginfo('version'), '5.5'), array(0, 1))) {
    add_filter( 'get_post_metadata', function( $value, $object_id, $meta_key, $single, $meta_type ) {
        if ( 0 === strpos($meta_key, 'tda_essential_css')) {
            global $paged;
...
if (wp_doing_ajax()) {
    if (!empty($_REQUEST['action']) && 'td_ajax_block' === $_REQUEST['action']) {
        //wp_set_current_user( 0 );
    }
}
